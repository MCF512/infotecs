...
let table = document.createElement('table'),
  ...
  headersArr = ["Name", "Last name", "About", "Eye color"],
  btnsArr = ["Hide/Show<br>Name", "Hide/Show<br>Last name", "Hide/Show<br>About", "Hide/Show<br>eye color"],
  currentPage = 1,
  rowsPerPage = 10,
  next__page = document.createElement('button'),
  prev__page = document.createElement('button'),
  page__p = document.createElement('p'),
  pages__wrapper = document.createElement('div'),
  rowIndex;

//Выводим кнопки для показа/скрытия колонок
btnsArr.forEach((item, index) => {
  let btn = document.createElement('button');
  btn.setAttribute('onclick', `showOrHideColumn(${index})`);
  btn.innerHTML = item;
  btn.classList.add('hide__btn')
  btnsDiv.append(btn);
});
btnsDiv.classList.add('btn__div');
body.append(btnsDiv);

//Создаем строку с заголовками
headersArr.forEach((item, index) => {
  let th = document.createElement('th');
  th.setAttribute('onclick', `sortColumn(${index})`);
  th.innerHTML = item;
  tr.append(th);
});
tbody.append(tr);
...
//Создаем div с формой для редактирования данных строки
headersArr.forEach(item => {
  let textarea = document.createElement('textarea');
  let label = document.createElement('label');
  label.innerHTML = item;
  //Для CSS
  textarea.setAttribute('data-attr', item);
...
next__page.innerHTML = 'Next';
next__page.classList.add('next');
prev__page.innerHTML = 'Prev';
prev__page.classList.add('prev')
//Изначально ставим кнопку prev в положение disbled, так как страница не может быть меньше 1
prev__page.setAttribute('disabled', 'disabled')
page__p.innerHTML = currentPage;
pages__wrapper.classList.add('pages__wrapper')
pages__wrapper.append(prev__page, page__p, next__page);
body.append(pages__wrapper);

next__page.addEventListener('click', () => {
  //Увеличиваем страницу и выводим новые данные
  currentPage++;
  ...
  changeData();
  //Если последняя страница, отключаем кнопку next
  if (currentPage == Math.ceil(json.length / rowsPerPage)) {
    next__page.setAttribute('disabled', 'disabled')
  }
  //Если кнопка prev была отключена - включаем её
  if (prev__page.hasAttribute('disabled')) prev__page.removeAttribute('disabled');
  //Скрываем div с формой изменения данных при переключении страницы
  if (div.style.display == 'block') div.style.display = 'none'
});

prev__page.addEventListener('click', () => {
  currentPage--;
  //обнуляем счетчик нажатой строки
  rowIndex = 0;
  //текущая страница / всего страниц
  page__p.innerHTML = currentPage;
  //Выводим обновленные данные
  ...
  //Добавляем возможность изменять данные
  changeData();
  if (currentPage == 1) prev__page.setAttribute('disabled', 'disabled');
  //Если кнопка next была отключена - включаем её
  if (next__page.hasAttribute('disabled')) next__page.removeAttribute('disabled');
  //Скрываем div с формой изменения данных при переключении страницы
  if (div.style.display == 'block') div.style.display = 'none'
});
...
submit.addEventListener('click', (e) => {
  ...
  //Предотвращаем перезагрузку страницы при нажатии
  e.preventDefault();
  //Получаем теги td внутри строки с индексом "к"
  let rowItems = tr[rowIndex].children;
  //Меняем содержимое строки таблицы, содержимым textarea
  for (let index = 0; index < rowItems.length; index++) {
    rowItems[index].innerHTML = textareas[index].value;

    if (index == 3) changeEyeColor(rowItems[index], rowItems[index].innerHTML)
  }
  //Обращение к строке json. Если находимся на первой странице, то json[rowIndex - 1], значение "rowIndex". Если не первая страница, то к переменной "rowIndex" нужно прибавить (номер текущей странцицы - 1) * максимальное число строк на странице
  let jsonNavigation = currentPage == 1 ? json[rowIndex - 1] : json[(rowIndex - 1) + ((currentPage - 1) * rowsPerPage)]
  //Изменяем значения json данными из полей textarea
  for (let n = 0; n < rowItems.length; n++) {
    switch (n) {
      case 0:
        jsonNavigation.name.firstName = textareas[n].value;
        break;
      case 1:
        jsonNavigation.name.lastName = textareas[n].value;
        break;
      case 2:
        jsonNavigation.about = textareas[n].value;
        break;
      case 3:
        jsonNavigation.eyeColor = textareas[n].value;
        break
    }
  }
  //Скрываем div после подтверждения
  div.style.display = 'none';
});
//Обработчик события сброса данных
reset.addEventListener('click', (e) => {
  ...
  //Обращение к строке json. Если находимся на первой странице, то json[rowIndex - 1], значение "rowIndex". Если не первая страница, то к переменной "rowIndex" нужно прибавить (номер текущей странцицы - 1) * максимальное число строк на странице
  let jsonNavigation = currentPage == 1 ? json[rowIndex - 1] : json[(rowIndex - 1) + ((currentPage - 1) * rowsPerPage)]
  //Сбрасываем изменение содержимого
  for (let n = 0; n < rowItems.length; n++) {
    switch (n) {
      case 0:
        textareas[n].value = jsonNavigation.name.firstName;
        break;
      case 1:
        textareas[n].value = jsonNavigation.name.lastName;
        break;
      case 2:
        textareas[n].value = jsonNavigation.about;
        break;
      case 3:
        textareas[n].value = jsonNavigation.eyeColor;
        break
    }
  }
});
...
//Функция для отображения данных из json
function showData() {
  let tr = document.querySelectorAll('tr');
  //Если json.length / rowsPerPage != 0, то на последней странице необходимо выводить меньше строк.
  let rowsOnPage = currentPage == Math.ceil(json.length / rowsPerPage) ? json.length - ((currentPage - 1) * rowsPerPage) : rowsPerPage;
  //Удаляем старые строки, чтобы выгрузить новые данные
  for (let i = 1; i < tr.length; i++) {
    tr[i].remove()
  }
  for (let i = 0; i < rowsOnPage; i++) {
    let tr = document.createElement('tr'),
      //Для правильного обращения по индексу. Если первая страница, то i от 0 до rowsOnPage. Если не первая, то (текущая страница * кол-во строк на странице) + i
      jsonNavigation = json[i + ((currentPage - 1) * rowsPerPage)],
      //массив с обращением к нужным данным из json
      dataFromJson = [jsonNavigation.name.firstName, jsonNavigation.name.lastName, jsonNavigation.about, jsonNavigation.eyeColor];

    //Выгружаем нужные данные в таблицу
    dataFromJson.forEach(data => {
      let td = document.createElement('td');
      td.innerHTML = data;
      //Для данных из "about" добавляем класс, в котором с помощью CSS ограничим вывод до 2 строк, остальное обрежем с помощью "..."
      if (data == jsonNavigation.about) td.classList.add('about');
      //Цвет глаз отображается цветом, а не текстом
      if (data == jsonNavigation.eyeColor) changeEyeColor(td, data);
      tr.append(td)
    });
    tbody.append(tr)
  }
}
// Функция изменения данных в строке
function changeData() {
  let trs = document.querySelectorAll('tr'),
    textareas = document.querySelectorAll('textarea');
  //В переменную "к" будем записывать индекс нажатой строки
  //Цикл начинается с 1, так как строка с индексом 0 - строка заголовков.
  for (let i = 1; i < trs.length; i++) {
    //row - одна строка, rowChild - теги td внутри row
    let row = trs[i];
    let rowChild = row.childNodes;
    //Обработик событий на каждую строку
    row.addEventListener('click', () => {
      //Показываем div с формой 
      if (div.style.display == 'none') {
        div.style.display = 'block'
      }
      //записываем индекс в переменную "rowIndex", чтобы обратиться к нему во время отправки или сборса данных
      rowIndex = i;
      //Заполняем поля textarea данными из строки
      textareas.forEach((area, index) => {
        area.value = rowChild[index].innerHTML
      });
    });
  }
};

//Функция для заливки фона и изменения цвета текста колонок Eye color
function changeEyeColor(tag, newColor) {
  tag.style.background = newColor;
  tag.style.color = newColor;
}

//функция показа/скрытия колонок.
function showOrHideColumn(n) {
  //HTMLcollection тегов tr
  let rows = table.rows;
  //Цикл перебора кажой строки
  for (let i = 0; i < rows.length; i++) {
    //elem - либо th, либо td. В строке с заголовками (индекс = 0) - th
    let elem = i == 0 ? rows[i].getElementsByTagName("TH")[n] : rows[i].getElementsByTagName("TD")[n];
    //Изменяем свойство display. Значение -webkit-box стоит для корректного отображения 3 колонки (about), в которой обрезаем лишнее и добавляем "..."
    if (elem.style.display == '' || elem.style.display == '-webkit-box') {
...
function sortColumn(n) {
  let rows, switching, i, firstTd, secondTd, shouldSwitch, direction, switchcount = 0;
  switching = true;
  /*Прописываем направления сортировки, при первом клики на заголовок элементы колонки будут отсортировани по алфавиту от a до z*/
  direction = "a-z";

  while (switching) {
    //Ставим switching в положение false, в теле цикла будем менять при условиях
    switching = false;
    //Получаем все строки (<tr></tr> теги)
    rows = table.rows;
    //Начинаем цикл перебора строк с 1, так как в 0 строке находятся заголовки таблицы
    for (i = 1; i < (rows.length - 1); i++) {
      shouldSwitch = false;
      /*Получаем значение тега td из нужной колонки и то же самое из следующей строки для дальнейшего их сравнения*/
      firstTd = rows[i].getElementsByTagName("TD")[n];
      secondTd = rows[i + 1].getElementsByTagName("TD")[n];
      /*Далее сравниваем 2 полученных значения, нужно ли менять их местами. Если нужно, ставим shouldSwitch в положение true и прерываем цикл for*/
      if (direction == "a-z") {
        if (firstTd.innerHTML.toLowerCase() > secondTd.innerHTML.toLowerCase()) {
          shouldSwitch = true;
          break;
        }
    ...
    /*После выхода из цикла, если shouldSwitch = true, меняем элементы местами и ставим switching в положение true, чтобы заново запустить цикл while*/
    if (shouldSwitch) {
      rows[i].parentNode.insertBefore(rows[i + 1], rows[i]);
      switching = true;
      //Увеличиваем значение каждый раз, когда выполняется замена местами
      switchcount++;
    } else {
      //Если все отсортировано, switchcount не будет изменен. Тогда, меняем направление сортировки и запускаем цикл while
      if (switchcount == 0 && direction == "a-z") {
        direction = "z-a";
        switching = true;
      }
    }
...
let instruction = `
Добрый день! Здесь написана небольшая инструкция: <br>
1. Для сортировки колонки необходимо нажать на заголовок требуемой колонки (Name, Last name, About, Eye color);<br>
2. Для скрытия/показа колонки необходимо нажать кнопку Hide/Show(Название колонки);<br>
3. Чтобы изменить данные в строке нужно нажать на строку, после чего под таблицей появляется div c формой редактирования данных. После заполнения полей новыми данными, нажать кнопку submit, чтобы сохранить данные в таблице и в json. Кнопка reset для сброса данных;<br>
4. Кнопки next/prev для перехода между страницами
`;
instructionDiv.innerHTML = instruction;
body.prepend(instructionDiv)